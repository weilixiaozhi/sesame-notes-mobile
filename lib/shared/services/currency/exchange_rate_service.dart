...
class ExchangeRateService {
  ...
  static final List<RateSource> sources = [
    RateSource(
      'fastly.jsdelivr.net',
      (b) => _fawazPath('fastly.jsdelivr.net', b),
    ),
    RateSource(
      'gcore.jsdelivr.net',
      (b) => _fawazPath('gcore.jsdelivr.net', b),
    ),
    RateSource(
  ...
  Future<RateFetchResult> fetch(String base) {
    ...
    final f = _fetchChain(base).whenComplete(() {
      _inflight.remove(
        key,
      ); // 丢弃返回值;Map.remove 返回 Future<V?>,若作为 whenComplete 返回值会造成循环等待。
    });
    _inflight[key] = f;
    return f;
  }
  ...
  Future<RateFetchResult> _fetchChain(String base) async {
    ...
    final start = (prefs.getInt(_prefKeyLastSource) ?? 0).clamp(
      0,
      sources.length - 1,
    );
    final errors = <String>[];
  ...
  static RateFetchResult parseFawaz(
    String base,

...
await provider.initialize({
  ...
  'remotePath': '/SesameNotes/', // optional
});
...
Use the unified storage interface via the provider:

```dart
import 'package:sesame_cloud_backup/sesame_cloud_backup.dart';

final storage = provider.storage;
await storage.upload(path: 'ledgers/123.json', data: jsonEncodedData);
final files = await storage.list(path: 'ledgers/');
...
Example with `remotePath: '/SesameNotes/'`:
- You specify: `ledgers/123.json`
- Actual path: `/SesameNotes/ledgers/123.json`
...
All operations throw standard exceptions from `sesame_cloud_backup`:

```dart
try {
  await provider.storage.upload(path: path, data: data);
